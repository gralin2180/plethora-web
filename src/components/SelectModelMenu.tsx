...
import { ManageModelsLink, ManageModelsModal } from "@/components/ManageModelsModal";
import {
  loadConnectedAi,
  setConnectedAccountModel,
  setPreferredAi,
} from "@/lib/connected-ai";
import { filterEnabledModels } from "@/lib/model-prefs";
import {
...
export function SelectModelMenu({
  ...
  useEffect(() => {
    ...
    window.addEventListener("plethora:models-updated", onModels);
    return () => window.removeEventListener("plethora:models-updated", onModels);
  }, []);
  ...
  const zenList = useMemo(() => {
    void modelTick;
    return filterModels(filterEnabledModels(OPENCODE_ZEN_FREE_MODELS), query);
  }, [query, modelTick]);
  const orList = useMemo(() => {
    void modelTick;
    return filterModels(filterEnabledModels(OPENROUTER_FREE_MODELS), query);
  }, [query, modelTick]);
  ...
  function pick(next: SelectedChatModel) {
    ...
    if (next.kind === "connected") {
      setPreferredAi(loadConnectedAi().preferred || "chatgpt");
    }
    setSelected(next);
    setOpen(false);
    setQuery("");
  ...
  return (
    <>
      <div ref={rootRef} className="relative">
        ...
        {open && (
          <div
            className={`absolute z-50 w-[min(100vw-2rem,20rem)] overflow-hidden rounded-xl border border-white/10 bg-[#16161c] shadow-2xl shadow-black/50 ${
              anchor === "down" ? "left-0 top-[calc(100%+8px)]" : "bottom-[calc(100%+8px)] left-0"
            }`}
          >
